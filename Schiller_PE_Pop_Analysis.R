# Compare S&P 500 Schiller P/E Ratio with population demographics

sp_500_schiller_pe <- read.csv("./sp_500_schiller_pe.csv")
us_pop_5year <- read.csv("./us_pop_5year.csv")

# round each year to the nearest 5 year mark
sp_500_schiller_pe$half_decade <- round(sp_500_schiller_pe$Year/5 + 0.01)*5
...
pe_pop_merged <- merge(sp_500_schiller_pe, us_age_props, by.x="half_decade", by.y="Year")

# compute a simple linear regression fit
linear_fit <- lm(Schiller_PE ~ X20_34 + X35_49 + X50_64 + over_64, data=pe_pop_merged)
summary(linear_fit)
...
# compute smoothed log-transformed Schiller P/E Ratios
pe_pop_merged$log_PE <- log_transform(pe_pop_merged$Schiller_PE)

# plot Schiller P/E histograms with and without log-transform
hist(pe_pop_merged$Schiller_PE, main="Schiller P/E Ratio Distribution",
     xlab="Schiller P/E", ylab="Probability",breaks=10,prob=TRUE)
curve(dnorm(x,mean=mean(pe_pop_merged$Schiller_PE),
            sd=sd(pe_pop_merged$Schiller_PE)),col="blue",add=TRUE)
hist(pe_pop_merged$log_PE, main="Log-Transformed Schiller P/E Ratio Distribution",
     xlab="Log-Transformed Schiller P/E Ratio", ylab="Probability",breaks=10,prob=TRUE)
curve(dnorm(x,mean=mean(pe_pop_merged$log_PE),
            sd=sd(pe_pop_merged$log_PE)),col="blue",add=TRUE)

# try with smoothed log-transformed Schiller P/E var
...
plot(x=pe_pop_merged$Year, y=pe_pop_merged$Schiller_PE, main="P/E Ratio over Time",
...
# plot predictions
plot(y=preds_logmod,x=pe_pop_merged$Year, main="Predicted Schiller P/E (Log Model)",
     xlab="Year",ylab="Predicted Schiller P/E")

plot(y=preds,x=pe_pop_merged$Year, main="Predicted Schiller P/E (Base Model)",
     xlab="Year",ylab="Predicted Schiller P/E")
...
points(x=pe_pop_merged$Year, y=pe_pop_merged$Schiller_PE,col="blue")
legend("bottomright", legend = c("predicted","actual"), pch=1, col=c("red","blue"),horiz=FALSE)

plot(x=pe_pop_merged$Schiller_PE,y=preds_logmod, main="Predicted vs Actual (Log Model)",
     xlab="Actual Schiller P/E Ratio", ylab="Predicted Schiller P/E Ratio")
...
plot(y=preds_midage,x=pe_pop_merged$Year, main="Predicted and Actual Schiller P/E Ratios (35-49 only)",
     xlab="Year",ylab="P/E Ratio",col="red")
points(x=pe_pop_merged$Year, y=pe_pop_merged$Schiller_PE,col="blue")
...
cor(pe_pop_merged$Schiller_PE, pe_pop_merged$under_20)
cor(pe_pop_merged$Schiller_PE, pe_pop_merged$X20_34)
cor(pe_pop_merged$Schiller_PE, pe_pop_merged$X35_49)
cor(pe_pop_merged$Schiller_PE, pe_pop_merged$X50_64)
cor(pe_pop_merged$Schiller_PE, pe_pop_merged$over_64)
